...
//   return (
//     <>
//     <header className="fixed top-0 left-0 right-0 lg:top-4 lg:left-1/2 lg:transform lg:-translate-x-1/2 bg-white shadow-lg lg:rounded-xl z-50 lg:max-w-6xl lg:w-[95%] w-full">
...
// Định nghĩa kiểu cho một mục menu con
interface NavChild {
  name: string;
  href: string;
}

// Định nghĩa kiểu cho một mục menu chính
interface NavItem {
  name: string;
  href: string;
  children?: NavChild[];
...
// Định nghĩa kiểu cho thông tin ngôn ngữ
...
const Header: FC = () => {
  ...
  // Định nghĩa cấu trúc navigation bên trong component
  // để có thể truy cập hàm `t` và được cập nhật khi ngôn ngữ thay đổi.
  const navigation: NavItem[] = [
    ...
    { 
      ...
      href: '/about',
      children: [
        { name: t('navigation.about_dropdown.introduction'), href: '/about' },
        { name: t('navigation.about_dropdown.certificates'), href: '/about/certificates' },
        { name: t('navigation.about_dropdown.history'), href: '/about/history' },
      ]
    },
  ...
  const isActiveRoute = (href: string, children?: NavChild[]): boolean => {
  ...
  const handleLanguageChange = (langCode: string) => {
    // Ép kiểu để đảm bảo langCode là 'vi' hoặc 'en'
    ...
    <>
        ...
        <div className="flex items-center justify-between h-16 lg:h-20 px-4 lg:px-6">
          ...
          <div className="flex-shrink-0 order-2 lg:order-1">
            <Link href="/" className="block">
              <Image
                src="/images/logo.png"
                alt={t('home_page.about_section.title')} // Sử dụng key từ file JSON
                width={150}
                height={56}
                className="h-10 lg:h-14 w-auto"
                priority
          ...
          <div className="lg:hidden flex items-center space-x-2 order-3">
            <div className="relative">
              ...
              >
                <Image
                  src={languages.find(l => l.code === language)?.flag || languages[0].flag}
                  alt={t('header.language_menu_title')}
                  width={18}
                  height={12}
                  className="rounded"
                />
              ...
              {isLangDropdownOpen && (
                <div className="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg py-1 z-50">
                  {languages.map((lang) => (
                    ...
                    >
                      <Image src={lang.flag} alt={lang.name} width={18} height={12} className="rounded"/>
                      <span>{lang.name}</span>
                    </button>
                  ))}
                </div>
      ...
      <div className={`lg:hidden fixed inset-0 z-40 transition-all duration-300 ease-in-out ${isMenuOpen ? 'opacity-100 visible' : 'opacity-0 invisible'}`}>
        <div 
          ...
          onClick={() => setIsMenuOpen(false)}
        ></div>
        <div className={`absolute top-0 left-0 h-full w-80 bg-white shadow-xl transform transition-transform duration-300 ease-in-out ${isMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
            <div className="p-6 pt-20">
              {/* Header của menu */}
              <div className="flex items-center justify-between mb-6">
                <Image
                ...
                {navigation.map((item) => (
                  <li key={item.href}>
                    <div className="flex items-center justify-between">
                      <Link
                        href={item.href}
                        className={`block py-3 flex-grow font-medium transition-colors ${
                          isActiveRoute(item.href, item.children)
                            ? 'text-green-600 font-semibold'
                            : 'text-gray-700 hover:text-green-600'
                        }`}
                        onClick={() => { if (!item.children) setIsMenuOpen(false); }}
                      >
                        {item.name}
                      </Link>
                      {item.children && (
                          ...
                          <li key={child.href}>
                            <Link
                              ...
                              className={`block py-2 text-sm border-l-2 pl-3 transition-colors ${
                                pathname === child.href
                                  ? 'text-green-600 border-green-600 bg-green-50 font-medium'
                                  : 'text-gray-600 hover:text-green-600 border-gray-200 hover:border-green-600'
                              }`}
                              onClick={() => setIsMenuOpen(false)}
                            >
                              {child.name}
              ...
              {/* Language switcher trong mobile menu */}
              <div className="mt-8 pt-6 border-t border-gray-200">
                <h3 className="text-sm font-medium text-gray-700 mb-3">{t('header.language_menu_title')}</h3>
                <div className="space-y-2">
                  {languages.map((lang) => (
                    <button
                      key={lang.code}
        ...
        <div className="relative w-full max-w-2xl" onClick={e => e.stopPropagation()}>
          <form className="relative">
            <input
              ...
              autoFocus
            />
            <button type="submit" className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-500" aria-label={t('header.search_placeholder')}>
              <Search className="w-6 h-6"/>
            </button>
          </form>
        </div>
